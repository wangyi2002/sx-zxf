...
class AGFormerBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
                 ...
                 temporal_msm=False, mamba_dt_bias_mode="legacy_double"):
        super().__init__()
        ...
        if mixer_type == 'attention':
            ...
            self.mixer = MAM(d_model=dim,
                             d_state=8,
                             d_conv=3,
                             expand=1,
                             mode=mode,
                             temporal_msm=temporal_msm,
                             dt_bias_mode=mamba_dt_bias_mode
                             )
            # self.mixer = Mamba2(
...
class MotionAGFormerBlock(nn.Module):
    ...
    def __init__(self, dim, mod="mamba", mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
        ...
        # ST Attention branch
            self.spatial = AGFormerBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                             ...
                                             use_temporal_similarity=use_temporal_similarity,
                                             neighbour_num=neighbour_num,
                                             n_frames=n_frames, temporal_msm=temporal_msm,
                                              mamba_dt_bias_mode=mamba_dt_bias_mode)
    ...
    def forward(self, x):
        ...
        if self.hierarchical:
            ...
            B, T, J, C = x.shape
            # x_m = x.reshape(B, T * J, C)
            # x_mam = self.mamba_temporal(self.mamba_spatial(x))
            x = self.temporal(self.spatial(x))
            # x_mam = x_mam.reshape(B, T, J, -1)
            # x_mam = self.mamba_temporal_1(self.mamba_spatial_1(x_mam))
        ...
        #     alpha = torch.cat((x_mam, x_attn), dim=-1)
        #     # alpha = torch.cat((x_mam, x_graph), dim=-1)
        #     alpha = self.fusion(alpha)
        #     alpha = alpha.softmax(dim=-1)
        #     x = x_mam * alpha[..., 0:1] + x_mam * alpha[..., 1:2]
        # else:
...
def _test():
    ...
    model = MotionAGFormer(n_layers=30, dim_in=3, dim_feat=128, mlp_ratio=4, hierarchical=False,
