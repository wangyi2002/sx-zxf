...
class Fusion(Dataset):
    ...
    def prepare_data(self, path, train=True):
        ...
        # [FIX] H36M order: lhip=1,lknee=2,lankle=3, lsho=14,lelb=15,lwri=16
        #                    rhip=4,rknee=5,rankle=6, rsho=11,relb=12,rwri=13
        self.kps_left, self.kps_right = [1, 2, 3, 14, 15, 16], [4, 5, 6, 11, 12, 13]
        self.joints_left, self.joints_right = [1, 2, 3, 14, 15, 16], [4, 5, 6, 11, 12, 13]
...
class MPI3DHP(Dataset):
    def __init__(self, args, train=True):
        ...
        # [FIX] H36M order: lhip=1,lknee=2,lankle=3, lsho=14,lelb=15,lwri=16
        #                    rhip=4,rknee=5,rankle=6, rsho=11,relb=12,rwri=13
        self.left_joints = [1, 2, 3, 14, 15, 16]
        self.right_joints = [4, 5, 6, 11, 12, 13]
    ...
    def normalize_poses(self):
        ...
        if self.train:
            for pose_sequence in self.poses_3d: # pose_sequence dim is (T, J, 3)
                ...
                normalized_sequence = normalized_sequence - normalized_sequence[:, 0:1, :]
